...
def forward(inputs):
    ...
    #1x1650  =====  150 x 11
    # print(inputs)
    inputs = tf.reshape(inputs, shape=[-1, 27])
    # print(inputs)
    energy_value = []
    # 遍历每一个样本下的所有原子数
    Energy_total = 0
        ...
        for step, (x_train, y_train) in enumerate(train_db):
            ...
            with tf.GradientTape(persistent=True) as tape:
                for i in range(0,x_train.shape[0]):
                    ...
                    train_batch_loss += loss_func
                    train_db_loss += loss_func
            # print( '============= train_batch_loss_func value is {} ============='.format(train_batch_loss/len(x_train)))
            for i in range(0, x_train.shape[0]):
                Atom_class = atom_class(inputs=x_train[i])
                for i in Atom_class:
                    if i == 0:
                        # 损失函数对其权重求取梯度
                        gradients_C = tape.gradient(target=train_batch_loss, sources=model_C.train_weight_matrix_list)
                        # 梯度更新
                        # 优化器指定
                        model_C.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients_C, model_C.train_weight_matrix_list))
                    elif i == 1:
                        # 损失函数对其权重求取梯度
                        gradients_H = tape.gradient(target=train_batch_loss, sources=model_H.train_weight_matrix_list)
                        # 梯度更新
                        model_H.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients_H, model_H.train_weight_matrix_list))
                    elif i == 2:
                        # 损失函数对其权重求取梯度
                        gradients_N = tape.gradient(target=train_batch_loss, sources=model_N.train_weight_matrix_list)
                        # 梯度更新
                        model_N.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients_N, model_N.train_weight_matrix_list))
                    elif i == 3:
                        # 损失函数对其权重求取梯度
                        gradients = tape.gradient(target=train_batch_loss, sources=model_O.train_weight_matrix_list)
                        # 梯度更新
                        model_O.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients, model_O.train_weight_matrix_list))
                    elif i == 4:
                        # 损失函数对其权重求取梯度
                        gradients = tape.gradient(target=train_batch_loss, sources=model_Zr.train_weight_matrix_list)
                        # 梯度更新
                        model_Zr.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients, model_Zr.train_weight_matrix_list))
                    elif i == 5:
                        # 损失函数对其权重求取梯度
                        gradients = tape.gradient(target=train_batch_loss, sources=model_Au.train_weight_matrix_list)
                        # 梯度更新
                        model_Au.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients, model_Au.train_weight_matrix_list))
                    elif i == 6:
                        # 损失函数对其权重求取梯度
                        gradients = tape.gradient(target=train_batch_loss, sources=model_Pd.train_weight_matrix_list)
                        # 梯度更新
                        model_Pd.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients, model_Pd.train_weight_matrix_list))
                    elif i == 7:
                        # 损失函数对其权重求取梯度
                        gradients = tape.gradient(target=train_batch_loss, sources=model_Ir.train_weight_matrix_list)
                        # 梯度更新
                        model_Ir.optimizer.apply_gradients(
                            grads_and_vars=zip(gradients, model_Ir.train_weight_matrix_list))
        # 清空当前batch和损失值
        # 迭代20epoch验证一次数据
        if epoch % 20 ==0:
            for _, (x_val, y_val) in enumerate(val_db):
                ...
                val_db_loss += val_loss_mse.numpy()

            print('Epoch is {}, ============= train loss value is {}.'.format(
                        epoch, train_db_loss / len(train_db)
                    ))
            print('Epoch is {}, ============= val loss value is {}.'.format(
                        epoch,val_db_loss / len(val_db)
                    ))
        #
        # self.train_batch_loss = 0
        # self.val_batch_loss = 0
...
if __name__ == '__main__':
    ...
    model_C = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=0, epochs=200)
    model_H = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=1, epochs=200)
    model_N = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=2, epochs=200)
    model_O = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=3, epochs=200)
    model_Zr = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=4, epochs=200)
    model_Au = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=5, epochs=200)
    model_Pd = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=6, epochs=200)
    model_Ir = HighDimensionAuClusterNet(
        input_nodes=26, hidden_nodes=20, output_nodes=1, learning_rate=0.05, nn_number=7, epochs=50)
    ...
    for fold_idx, train_path in enumerate(sample_src):
        for filename in os.listdir(train_path):
            ...
            with open(filepath, mode='rb') as fp:
                # 加载每一个样本个体
                dataset = np.load(fp)
            sample, label = dataset[:, :27], dataset[:, -1][0]
            ...
            if sample.shape[0] < 150:
                zero_matrix = np.zeros(shape=[150 - sample.shape[0], 27])
    ...
    # print(samples)
    x_train, x_test, y_train, y_test = train_test_split(samples, labels, test_size=0.3, shuffle=True,
                                                        random_state=0)
    ...
    train_db = tf.data.Dataset.from_tensor_slices((x_train, y_train)).batch(30) #30 x 1650
...
# class PSO():
#     def __init__(self,featureNum,featureArea,featureLimit,featureType,particleNum=50,epochMax=1000,c1=2,c2=2):
...
#
#
#     def buildNet(nodeNum,batch,epoch,ln):
#         model = HighDimensionAuClusterNet(input_nodes=26, hidden_nodes=nodeNum, output_nodes=1, learning_rate=ln, nn_number=0, epochs=epoch)
#         samples = []
#         labels = []
#         for fold_idx, train_path in enumerate(sample_src):
#             for filename in os.listdir(train_path):
...
#         y_val = tf.cast(y_val, dtype=tf.float32)
#         # 数据批次打包
#         train_db = tf.data.Dataset.from_tensor_slices((x_train, y_train)).batch(batch)
#         test_db = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(batch_size=1)
#         val_db = tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(batch_size=1)
#         #训练模型
#         train(train_db,val_db);
#         #模型评估
#         for _, (x_val, y_val) in enumerate(val_db):
#             y_pred = model.forward(inputs=x_val)
#             y_pred = tf.cast(y_pred, dtype=y_val.dtype)
#             val_loss_mse = tf.reduce_mean(tf.square(y_val - y_pred))
#             val_rmes += val_loss_mse.numpy()
#         return val_rmes;
#
#
